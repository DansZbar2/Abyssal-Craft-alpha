...
function shotEntity(gun, vectorSpawn, vectorSpeed){
    ...
    Entity.setSkin(entity, gun.bullet.skin);
...
var granede_model = new EntityModel();
var granede_texture = new Texture();

function getGrenadeRender(){
    var Gmodel = new Render();
    var Gbody = [
        {
            type: "box",
            size: { x: 2, y: 4, z: 3},
            uv: { x: 0, y: 0},
            coords: { x: -1, y: 20, z: -1.5}
        }];
    var Ghead = [
        {
            type: "box",
            size: { x: 1, y: 1, z: 1},
            uv: { x: 8, y: 0},
            coords: { x: -0.5, y: 20.5, z: 1.5}
        }];
    
    Gmodel.setPart("head", Ghead, {});
    Gmodel.setPart("body", Gbody, {});
    return Gmodel;
}